...
type MergeEdit = { baseStart: number; baseEnd: number; replacement: string[] };

// ── Cooklang text manipulation (pure, no I/O) ────────────────────────────────
...
function entryRowToVersion(row: EntryRow): VersionRecord {
...
function branchRowToMeta(row: BranchRow): RecipeBranchMeta {
  return {
    id: row.id,
    slug: row.slug,
    name: row.name,
...
function cookLogRowToRecord(row: CookLogRow): CookLogRecord {
  return {
    id: row.id,
    recipe_id: row.recipe_id,
    branch_slug: row.branch_slug,
