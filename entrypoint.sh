...
if [ "${AUTO_PULL:-true}" = "true" ] && [ -d ".git" ]; then
  echo "[recipevault] Pulling latest code..."
  BRANCH="$(git rev-parse --abbrev-ref HEAD 2>/dev/null || echo main)"
  git pull origin "$BRANCH" || echo "[recipevault] Warning: git pull failed — continuing with existing code"
fi

# Install/update Node dependencies.
# Needed on first start when node_modules volume is empty, and after pulls that
# change package.json.
echo "[recipevault] Installing dependencies..."
npm install --silent

# Rebuild the CodeMirror vendor bundle.
echo "[recipevault] Building editor bundle..."
...
echo "[recipevault] Starting server..."
exec npm start
